...
class TestOverallAnalysis(unittest.TestCase):
    ...
    def test_get_similar_structures(self):
        ...
        self.assertDictEqual(
            dict((self.overallanalysis._get_similar_structures(["mp-7000", "mp-1788", "mp-553432_only_for_testing"],
                                                               start_from_Matching=True,
                                                               save_to_file=False, tight=False)))[
                "structure_matching"], {'mp-7000': ['mp-7000', 'mp-553432_only_for_testing'], 'mp-1788': ['mp-1788']})
...
class TestPauling2OverAllAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        self.pauling2_normal = Pauling2OverAllAnalysis(source='my_own_list', onlybinaries=False,
                                                       ...
                                                       list_of_materials_to_investigate='test_list.json',
                                                       analyse_structures=True, use_prematching=True)
        self.pauling2_optmized_environments = Pauling2OverAllAnalysis(source='my_own_list', onlybinaries=False,
                                                                      plot_element_dependend_analysis=False,
                                                                      list_of_materials_to_investigate='test_list.json',
                                                                      analyse_structures=True, use_prematching=True)
    ...
    def test_run(self):
        self.pauling2_normal.run(start_from_results=False, save_result_data=False, path_to_save='',
                                 show_plot=False,
                                 save_structure_analysis=False, stepsize_histogram=0.01)
        ...
        self.assertDictEqual(self.pauling2_normal.dict_similarstructures_fulfilling,
                             {'list_mat_id': ['mp-19359', 'mp-2605', 'mp-7000', 'mp-306'],
                              'structure_matching': OrderedDict(
                                  [('mp-19359', ['mp-19359', 'mp-2605']), ('mp-7000', ['mp-7000']),
                                   ('mp-306', ['mp-306'])]),
        ...
        comparedict4 = [-0.16666667, -0.16166667, -0.15666667, -0.15166667, -0.14666667, -0.14166667, -0.13666667,
                        -0.13166667, -0.12666667, -0.12166667, -0.11666667, -0.11166667,
                        -0.10666667, -0.10166667, -0.09666667, -0.09166667, -0.08666667, -0.08166667, -0.07666667,
                        -0.07166667, -0.06666667, -0.06166667, -0.05666667, -0.05166667,
                        -0.04666667, -0.04166667, -0.03666667, -0.03166667, -0.02666667, -0.02166667,
                        -0.01666667, -0.01166667, -0.00666667, -0.00166667, 0.00333333, 0.00833333,
                        0.01333333, 0.01833333, 0.02333333, 0.02833333, 0.03333333, 0.03833333,
                        0.04333333, 0.04833333, 0.05333333, 0.05833333, 0.06333333, 0.06833333,
                        0.07333333, 0.07833333, 0.08333333, 0.08833333, 0.09333333, 0.09833333,
                        0.10333333, 0.10833333, 0.11333333, 0.11833333, 0.12333333, 0.12833333]
        for ivalue, value in enumerate(list(self.pauling2_normal.arraydev_share_plus_minus)):
            self.assertAlmostEqual(value, comparedict4[ivalue])

        # check if the number of anions keeps the same after processing
        self.assertEqual(sum(self.pauling2_normal.frequency_plus_minus), len(self.pauling2_normal.array_bvs))
        ...
        self.pauling2_optmized_environments.run(start_from_results=False, save_result_data=False, path_to_save='',
                                                show_plot=False,
                                                save_structure_analysis=False, stepsize_histogram=0.01,
                                                optimized_environments=True, fraction_environment=0.3)
        self.assertListEqual(self.pauling2_optmized_environments.structures_fulfillingrule,
                             ['mp-7000', 'mp-19359', 'mp-306', 'mp-2605'])
        self.assertListEqual(self.pauling2_optmized_environments.structures_cannot_be_evaluated, [])
        self.assertListEqual(self.pauling2_optmized_environments.structures_exceptions, ["mp-1788", "mp-886"])
        self.assertListEqual(self.pauling2_optmized_environments.array_bvs, self.pauling2_normal.array_bvs)
    ...
    def test_get_deviation_from_ideal_value(self):
        ...
        ideal = 0
        self.assertListEqual(self.pauling2_normal._get_absolute_deviation_from_ideal_value(list_here, ideal),
                             [0.5, 1.0, 2.0, 3.0, 0.0])
        ideal = 1.0
        self.assertListEqual(self.pauling2_normal._get_absolute_deviation_from_ideal_value(list_here, ideal),
                             [0.5, 0.0, 3.0, 2.0, 1.0])
    ...
    def test_get_cases_with_extreme_deviations(self):
        # TODO: correct this!
        addi = {"mp-3400": [0, 4, 2, 2], "mp-4000": [2, 2, 2, 2], "mp-3000": [2.2, 2, 2], "mp-3001": [2.21, 2, 2],
                "mp-700": [1.8, 2, 2], "mp-600": [1.79, 2, 2]}
        exceptions = self.pauling2_normal._get_extreme_exceptions(additional_info=addi, perc_std=0.1, ideal_value=2,
                                                                  larger_how_many_std=1)
        self.assertListEqual(exceptions, ["mp-3400", "mp-3001", "mp-600"])
...
class TestPauling3OverAllAnalysis(unittest.TestCase):
    ...
    def test_run(self):
        ...
        self.assertDictEqual(self.pauling3normal.dict_similarstructures_fulfilling,
                             {'list_mat_id': ['mp-19359', 'mp-2605', 'mp-1788', 'mp-7000', 'mp-306', 'mp-886'],
                              'structure_matching': OrderedDict(
                                  [('mp-19359', ['mp-19359', 'mp-2605']), ('mp-1788', ['mp-1788']),
                                   ('mp-7000', ['mp-7000']),
                                   ('mp-306', ['mp-306']), ('mp-886', ['mp-886'])]),
                              'additional_info': {'mp-1788': 'As2O5', 'mp-7000': 'SiO2', 'mp-19359': 'NaFeO2',
                                                  'mp-306': 'B2O3',
                                                  'mp-886': 'Ga2O3', 'mp-2605': 'CaO'}})
...
class TestPauling4OverAllAnalysis(unittest.TestCase):
    ...
    def test_run(self):
        ...
        self.assertDictEqual(self.pauling4normal4.Dict_CN,
                             {'6': {'6': [20, 6], '12': [48, 16]}, '12': {'6': [48, 16], '12': [8, 18]}})
        self.assertDictEqual(self.pauling4normal4.Dict_val,
                             {'4': {'4': [20, 6], '2': [48, 16]}, '2': {'4': [48, 16], '2': [8, 18]}})
        ...
        self.assertEqual(self.pauling4normal2.Dict_val["5"]["5"][1],
                         self.pauling4normal2.Dict_CN["4"]["4"][1] + self.pauling4normal2.Dict_CN["4"]["6"][1] +
                         self.pauling4normal2.Dict_CN["6"]["6"][1])

        self.assertDictEqual(self.pauling4normal2.Dict_CN,
                             {'4': {'4': [88, 0], '6': [192, 32]}, '6': {'4': [192, 32], '6': [96, 8]}})
        ...
        self.assertDictEqual(self.pauling4normal3.Dict_val,
    ...
    def test_reformatting(self):
        ...
        self.assertDictEqual(self.pauling4normal._reformat_details_CN({'val1:1': {
            'val2:1': {'CN1:6': {'CN2:6': {'no': 18, 'corner': 0, 'edge': 3, 'face': 0},
                                 'CN2:3': {'no': 30, 'corner': 6, 'edge': 6, 'face': 0}}},
            'val2:3': {'CN1:6': {'CN2:6': {'no': 30, 'corner': 6, 'edge': 6, 'face': 0},
                                 'CN2:3': {'no': 30, 'corner': 6, 'edge': 6, 'face': 0}}}}, 'val1:3': {
            'val2:1': {'CN1:6': {'CN2:6': {'no': 30, 'corner': 6, 'edge': 6, 'face': 0}}},
            'val2:3': {'CN1:6': {'CN2:6': {'no': 18, 'corner': 0, 'edge': 3, 'face': 0}}}}}),
            {'6': {'6': [132, 36], '3': [60, 24]}, '3': {'6': [60, 24]}})
        ...
        self.assertDictEqual(self.pauling4normal._reformat_details_val(
            {'val1:1': {
                ...
                'val1:3': {
                    'val2:1': {'CN1:6': {'CN2:6': {'no': 30, 'corner': 6, 'edge': 6, 'face': 0}}},
                    'val2:3': {'CN1:6': {'CN2:6': {'no': 18, 'corner': 0, 'edge': 3, 'face': 0}}}}}),
            {'1': {'1': [66, 18], '3': [90, 36]}, '3': {'1': [90, 36], '3': [36, 6]}})
